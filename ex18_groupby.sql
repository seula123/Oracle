...
-- group by 사용 시 > select 컬럼리스트 > 일반 컬럼 명시 불가능
-- 집계 함수 + 그룹컬럼만 사용 가능
SELECT
   count(*), name
FROM tblinsa
...
/*
    ...
    group by 기준         3. (레코드끼리) 그룹을 나눈다.
    having 조건            4. 조건 지정(그룹에 대한 조건 - 그룹 조건 > 집계 함수)-GROUP BY에 조건문을 사용하려면 HAVING절 사용
...
FROM tblinsa               --1. 60명의 데이터를 가져온다.
   ...
   GROUP BY buseo;            --3. 2번을 통과한 사람등(27명) 대상으로 그룹을 짓는다.
      
SELECT
   buseo,
   round(avg(basicpay))
FROM tblinsa                        --1. 60명의 데이터를 가져온다.
   GROUP BY buseo                     --2. 그룹을 짓는다.
      HAVING avg(basicpay) >= 1500000;   --3. 그룹에 대한 조건 > 집계 함수 조건
...
--substr(컬럼, 시작위치, 문자개수)
SELECT substr(address,1, instr(address, ' ')-1) FROM tbladdressbook; --공백을 기준으로경기도,서울특별시
...
SELECT
	...
	MIN(basicpay)
FROM tblinsa
	GROUP BY ROLLUP(buseo,jikwi);  --ROLLUP중간 결산 값 보여주는 역할
...
--cube > 다중 그룹 컬럼 > 수평관계 (전체인원 총계, 부서별 총계, 직위별 통계)
