...
--5 null 컬럼 조작
--a null 상수
INSERT INTO tblMemo (seq, name, memo, regdate,name) 
...
INSERT INTO tblMemo (seq, memo, regdate) 
            VALUES (seqMemo.nextVal, '메모입니다.', sysdate);		
--6.b null 상수 null대입(개발자의지 표현) > default 동작 안함
INSERT INTO tblMemo (seq, name, memo, regdate) 
            VALUES (seqMemo.nextVal, '홍길동', '메모입니다.', sysdate);		
--6.c default 상수 
INSERT INTO tblMemo (seq, name, memo, regdate) 
...
--컬럼 리스트를 생략하면 테이블의 원본 컬럼 순서대로 값리스트를 작성해야한다.
...
--9
--tblMemo 테이블 > 복사 > 새테이블 생성(tblMemoCopy2)
-- 제약 사항이 복사가 안된다.
-- 임시 테스트용으로만 사용 > 다량의 데이터, 임시 데이터를 잠시 ㅅ용할때
