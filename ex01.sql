...
--
--Oracale Database
---제품명이자 회사명이다 오라클회사에서 오라클을 판다
--
--
--Database
--- 데이터베이스 = 데이터 집합
--- 데이터 베이스 관리 시스템: 관리,운영,보안 하기 위해서 시스템을 만들어놓은것
--
--데이터 베이스 관리 시스템
--= Database Managemetn System
---그 중 하나가 오라클
---오라클(DB + DBMS =데이터를 모아놓고 조작까지 할수있음(관리))
--
--
--백그라운드 프로세스
--= 서비스(라고 부른다)(Service)
...
--- 개발자 - 중계인(데이터 베이스 클라이언트) - 오라클(데이터베이스 서버)
--
--플러스-새데이터베이스접속선택
--
--
--편집 창 > 워크 시트 > 스크립트 (파일) >*.sql 
...
system 작업 안함 - 권한이 너무 커서;; - 보안,실수할까봐

일반 계정 작업 = 특정 업무에 대한 권한이 있는 계정 > 수업


학습용 계정 제공 (=일반 계정) > + 샘플 데이터 같이 준다 > hr
...
오라클 수업 = ANSI-SQL(5~60%) + PL/SQL(2~30%) + (데이터베이스)설계,기타(10%)
