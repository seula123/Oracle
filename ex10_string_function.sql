...
/*
 ...
 - varchar2 substr(컬럼, 시작위치)  --숫자 안 넣어주면 끝까지 가져옴
...
SELECT 
	...
	substr(ssn, 3,4) AS 월일,
	substr(ssn, 5,2) AS 생일,
	substr(ssn, 8,1) AS 생일 
FROM tblinsa;
...
SELECT
   ...
   END) AS 김,
      count(CASE
      WHEN substr(name, 1, 1) = '이' THEN 2      
   END) AS 이,
      count(CASE
      WHEN substr(name, 1, 1) = '박' THEN 3      
   END) AS 박,
      count(CASE
      WHEN substr(name, 1, 1) = '최' THEN 4      
   END) AS 최,
      count(CASE
      WHEN substr(name, 1, 1) = '정' THEN 5      
   END) AS 정,
   count(CASE
...
SELECT
	...
	END AS 성별,
	REPLACE(replace(gender, 'm', '남자'), 'f', '여자') AS 성별,
	decode(gender, 'm', '남자', 'f', '여자') AS 성별 --GOOD 
FROM tblcomedian;
