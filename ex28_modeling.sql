...
------------------------------------------
	...
	모델링 작업 > ERD > 올바르게했는지 검증하는 작업을 정규화라고 한다 > 안정성 높고, 작업하기 편한 ERD
	
	
	정규화, Normallization
	- 자료의 손실이나, 불필요한 정보를 없애고, 데이터의 일관성을 유지하며, 데이터의 종속성을 최소화해주기 위해 ERD를 수정하는 작업
	- 우리가 만든 테이블(ERD)를 비정형,비정규화 상태라고 한다. > 정규화 상태
	...
	2. 중복값 제거
	3. 복합값 제거
	4. 자료의 삽입 이상, 갱신 이상, 삭제 이상현상을 제거한다.
	
	
	이상 현상
	1. 삽입 이상, Insertion Anomaly
		- 특정 테이블에 데이터를 삽입할 때 원하지 않는 데이터까지 같이 넣어야하는 상황
		
	2. 갱신 이상, Update Anomaly
		- 동일한 데이터가 2개 이상의 테이블에 동시 저장
			> 그 중 1개는 수정했지만, 다른 1개를 수정하지 못하면 둘 중 어느것이 올바른 데이터인지 판단하지 못하는 상황
