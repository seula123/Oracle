...
-- 로그인 직후(접속) > 트랜잭션이 시작됨
-- 트랜잭션 > 모든 명령어(X) > INSERT, UPDATE, DELETE 멍령어만 트랜잭션에 포함된다.
-- INSERT, UPDATE, DELETE 작업 > 오라클 적용(X), 임시 메모리 적용(O)

DELETE FROM tbltrans WHERE name = '박문수';	-- 트랜잭션에 포함

SELECT * FROM tbltrans;	-- 트래잭션과 무관

COMMIT;
ROLLBACK; --현재 트랜잭션에서 했던 모든 행동을 진짜로 데이터베이스에 적용하지 말고 취소
...
/*
 ...
 3. 클라이언트 접속 종료
 	a. 정상 종료
 		- 현재 트랜잭션에 아직 반영이 안 된 명령어가 남아있는데 어떻게 하실?  
 	b. 비정상 종료
